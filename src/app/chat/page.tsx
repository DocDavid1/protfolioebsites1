...
const Blockquote: React.FC<React.BlockquoteHTMLAttributes<HTMLElement>> = (
  ...
  <blockquote
    className="mb-3 border-l-2 border-gray-200 dark:border-white/[0.08] pl-3 text-gray-500 dark:text-white/50"
    {...props}
  />
);
const Code: Components["code"] = ({ children, className, ...props }) => {
  ...
  if (isInline) {
    return (
      <code className="rounded bg-gray-100 dark:bg-white/[0.04] px-1 py-0.5 text-xs" {...props}>
        {children}
      </code>
    );
  }
  return (
    <pre className="mb-3 w-full overflow-x-auto rounded-md bg-gray-100 dark:bg-white/[0.04] p-3">
...
const HR: React.FC<React.HTMLAttributes<HTMLHRElement>> = (props) => (
  <hr className="my-4 border-gray-200 dark:border-white/[0.08]" {...props} />
);
...
const TH: React.FC<React.ThHTMLAttributes<HTMLTableCellElement>> = (props) => (
  <th
    className="border border-gray-200 dark:border-white/[0.08] bg-gray-100 dark:bg-white/[0.04] px-2 py-1 text-left"
    {...props}
  />
);
const TD: React.FC<React.TdHTMLAttributes<HTMLTableCellElement>> = (props) => (
  <td className="border border-gray-200 dark:border-white/[0.08] px-2 py-1" {...props} />
);
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="p-1 hover:bg-gray-100 dark:hover:bg-white/[0.06] rounded transition-colors"
      title="העתק ללוח"
    >
      {copied ? (
        <Check className="h-3.5 w-3.5 text-green-500" />
      ) : (
        <Copy className="h-3.5 w-3.5 text-gray-500 dark:text-white/50" />
      )}
    </button>
  );
}
...
function ThinkingIndicator() {
  return (
    <div className="flex items-center gap-2 p-3 rounded-lg bg-gray-100 dark:bg-white/[0.04] max-w-[80%]">
      <Loader2 className="h-4 w-4 animate-spin" />
      <span className="text-sm text-gray-500 dark:text-white/50">AI חושב...</span>
    </div>
  );
}
  ...
  return (
    <div className="relative overflow-hidden container mx-auto px-4 py-8">
      ...
      <div className="relative max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-200 dark:border-white/[0.08] backdrop-blur-md bg-gray-50 dark:bg-white/[0.02] rounded-xl px-4 py-3">
              ...
              <div
                ...
                className={`group p-3 rounded-lg ${
                  message.role === "user"
                    ...
                    : "bg-gray-100 dark:bg-white/[0.04] max-w-[80%]"
                }`}
              >
        ...
        >
          <input
            ...
            className="flex-1 p-2 border border-gray-200 dark:border-white/[0.08] rounded-md bg-gray-100 dark:bg-white/[0.04] text-white placeholder:text-gray-400 dark:text-white/30 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50"
